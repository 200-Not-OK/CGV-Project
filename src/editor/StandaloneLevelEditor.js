...
this.collectibleMeshes = [];
  ...
  _positionCameraForLevel() {
    ...
    console.log(
      `Camera positioned at: ${this.camera.position
        .toArray()
        .map((v) => v.toFixed(1))
        .join(", ")}`
    );
    console.log(
      `Looking at level center: ${center
        .toArray()
        .map((v) => v.toFixed(1))
        .join(", ")}`
    );
  ...
  _addPatrolBehaviorIndicators(group) {
    ...
    console.log(
      `🧭 Patrol route for ${group.type} has ${group.points.length} points, behavior: ${group.behavior}`
    );
  }
  ...
  _createPatrolConnections() {
    ...
    this.patrolPoints.forEach((point) => {
      const key =
        point.entityType === "npc"
          ? `npc_${point.npcIndex}`
          : `enemy_${point.enemyIndex}`;
      if (!patrolGroups[key]) {
        patrolGroups[key] = {
          points: [],
  ...
  _createPlatformVisuals() {
    ...
    this.platforms.forEach((platform, index) => {
      ...
      if (
        ...
        platform.animation?.path &&
        platform.animation.path.length > 1
      ) {
  ...
  _createPlatformMaterial(platform) {
    ...
    if (textureUrl) {
      ...
      if (!this.platformTextures[textureUrl]) {
        ...
        texture.repeat.set(
          platform.textureRepeat[0],
          platform.textureRepeat[1]
        );
...
// Add to _bindUIEvents() method:
const addCollectibleBtn = document.getElementById('add-collectible');
if (addCollectibleBtn) {
  addCollectibleBtn.addEventListener('click', () => {
    this.mode = 'collectible';
    this._updateStatus('Click on the level to place a collectible');
  });
}
  ...
  _updateUI() {
    ...
    console.log(
      "_updateUI called, mode:",
      this.mode,
      "selectedMesh:",
      this.selectedMesh
    );

    const levelSelect = this.levels
      .map(
        (level, index) =>
          `<option value="${index}" ${
            index === this.currentLevelIndex ? "selected" : ""
          }>${level.name}</option>`
      )
      .join("");

    const enemyTypeOptions = this.enemyTypes
      .map(
        (type) =>
          `<option value="${type}">${
    ...
    const lightTypeOptions = this.lightTypes
      .map((type) => `<option value="${type}">${type}</option>`)
      .join("");

    const colliderTypeOptions = this.colliderTypes
      .map(
        (type) =>
          `<option value="${type}">${
            type.charAt(0).toUpperCase() + type.slice(1)
    ...
    const materialTypeOptions = this.materialTypes
      .map(
        (type) =>
          `<option value="${type}">${
            type.charAt(0).toUpperCase() + type.slice(1)
    ...
    this.panel.innerHTML = `
      ...
      <div style="margin-bottom: 20px;">
        ...
        <button id="mode-enemy" class="mode-btn ${
          this.mode === "enemy" ? "active" : ""
        }">Enemies</button>
        <button id="mode-npc" class="mode-btn ${
          this.mode === "npc" ? "active" : ""
        }">NPCs</button>
        <button id="mode-light" class="mode-btn ${
          this.mode === "light" ? "active" : ""
        }">Lights</button>
        <button id="mode-patrol" class="mode-btn ${
          this.mode === "patrol" ? "active" : ""
        }">Patrol</button>
        <button id="mode-platform" class="mode-btn ${
          this.mode === "platform" ? "active" : ""
        }">Platforms</button>
        <button id="mode-interactive" class="mode-btn ${
          this.mode === "interactive" ? "active" : ""
        }">Interactive</button>
        <button id="mode-block" class="mode-btn ${
          this.mode === "block" ? "active" : ""
        }">Blocks</button>
        <button id="mode-mesh" class="mode-btn ${
          this.mode === "mesh" ? "active" : ""
        }">Meshes</button>
        <button id="mode-collider" class="mode-btn ${
          this.mode === "collider" ? "active" : ""
        }">Colliders</button>
        <button id="mode-collectible" class="mode-btn ${this.mode === "collectible" ? "active" : ""}">Collectibles</button>
        <button id="mode-select" class="mode-btn ${
          this.mode === "select" ? "active" : ""
        }">Select</button>
      </div>
      
      <div id="enemy-controls" style="display: ${
        this.mode === "enemy" ? "block" : "none"
      };">
      ...
      <div id="npc-controls" style="display: ${
        this.mode === "npc" ? "block" : "none"
      };">
        <h4>NPC Controls</h4>
        <label>Type:</label><br>
      ...
      <div id="light-controls" style="display: ${
        this.mode === "light" ? "block" : "none"
      };">
      ...
      <div id="block-controls" style="display: ${
        this.mode === "block" ? "block" : "none"
      };">
      ...
      <div id="mesh-controls" style="display: ${
        this.mode === "mesh" ? "block" : "none"
      };">
        ...
        <div id="mesh-list">
          ...
          ${this.levelMeshes
            .map(
              (meshInfo, index) => `
            <div class="item-row ${
              this.selectedMesh === meshInfo ? "selected" : ""
            }" 
        ...
        ${
          ...
          <div style="margin-top: 15px; padding: 10px; border: 1px solid #4CAF50; background: #1a1a1a;">
            <h5 style="color: #4CAF50;">✓ Selected: ${
            ...
            ${
              ...
              <div style="margin-top: 15px; padding: 10px; border: 1px solid #FF9800; background: #1a1a1a; border-radius: 3px;">
                <h6 style="margin: 0 0 8px 0; color: #FF9800;">📋 Mesh Animations (${
                  this.meshAnimations.length
                })</h6>
                ${this.meshAnimations
                  .map(
                    (anim, index) => `
                  <div style="font-size: 10px; margin: 5px 0; padding: 5px; background: #222; border-radius: 2px; display: flex; justify-content: space-between; align-items: center;">
                    <div>
                      ...
                      ${
                        anim.animationType === "moving"
                          ? `<br>Waypoints: ${anim.data.path?.length || 0}`
                          : ""
                      }
                    </div>
                    <button onclick="window.editor._deleteMeshAnimation('${
                      anim.meshName
                    }')" 
      ...
      <div id="collider-controls" style="display: ${
        this.mode === "collider" ? "block" : "none"
      };">
      ...
      <div id="platform-controls" style="display: ${
        ...
        <div id="platform-list">
          ...
          ${this.platforms
            ...
            <div class="item-row" data-type="platform" data-index="${index}">
              <strong>${platform.type}</strong> ${platform.size[0]}x${
                platform.size[1]
              }x${platform.size[2]}<br>
      ...
      <div id="interactive-controls" style="display: ${
        this.mode === "interactive" ? "block" : "none"
      };">
        ...
        <div id="interactive-list">
          <h5>Interactive Objects (${
            this.interactiveObjects ? this.interactiveObjects.length : 0
          })</h5>
          ${
            this.interactiveObjects
              ? this.interactiveObjects
        ...
        <div id="trigger-list">
          <h6 style="margin: 5px 0;">Triggers (${
  ...
  <select id="collectible-type" style="width: 100%; padding: 5px; margin-bottom: 10px;">
  ...
  <button id="add-collectible" style="width: 100%; padding: 5px;">Add Collectible (Click on level)</button>
  ...
  <div id="collectible-list">
    <h5>All Collectibles</h5>
    ${this._getCollectiblesListHTML()}
  </div>
</div>
      
      <div id="patrol-controls" style="display: ${
        this.mode === "patrol" ? "block" : "none"
      };">
        ...
        <div id="patrol-list">
          ...
          ${this.patrolPoints
            ...
            <div class="item-row" data-type="patrol" data-index="${index}">
              ${point.entityType === "npc" ? "NPC" : "Enemy"} ${
                point.entityType === "npc" ? point.npcIndex : point.enemyIndex
  ...
  _getSelectionPropertiesHTML() {
    ...
    if (this.selectedType === "enemy") {
      ...
      typeSpecificInputs = `
        ...
        <div style="margin-top: 5px;">
          ...
          <input type="number" id="selected-speed" value="${
            enemyData.speed || 2
          }" step="0.1" min="0" 
        ...
        <div style="margin-top: 5px;">
          ...
          <input type="number" id="selected-chase-range" value="${
            enemyData.chaseRange || 5
          }" step="0.5" min="0" 
                 style="width: 100%; padding: 3px; font-size: 11px;" 
                 onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
                 oninput="window.editor._onPropertyInputChange()"
      ...
      `;
    } else if (this.selectedType === 'collectible') {
      const collectibleData = selectedData.collectibleData;
      const collectibleType = selectedData.type;
      
      typeSpecificInputs = `
        <div style="margin-top: 10px;">
          <label style="font-size: 11px;">Collectible Type:</label><br>
          <span style="font-size: 11px; color: #4CAF50;">${collectibleType.charAt(0).toUpperCase() + collectibleType.slice(1)}</span>
        </div>
        ${collectibleType === 'chest' ? `
          <div style="margin-top: 5px;">
            <label style="font-size: 11px;">Chest Contents:</label><br>
            <span style="font-size: 11px; color: #4CAF50;">${collectibleData.contents}</span>
          </div>
        ` : ''}
      `;
    }
      ...
      typeSpecificInputs = `
        ...
        <div style="margin-top: 5px;">
          <label style="font-size: 11px;">Speed:</label><br>
          <input type="number" id="selected-npc-speed" value="${
            npcData.speed || 2
          }" step="0.1" min="0" 
                 style="width: 100%; padding: 3px; font-size: 11px;" 
                 onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
        ...
        <div style="margin-top: 5px;">
          ...
          <input type="number" id="selected-npc-scale" value="${
            npcData.scale || 1.0
          }" step="0.1" min="0.1" 
                 style="width: 100%; padding: 3px; font-size: 11px;" 
                 onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
        ...
        <div style="margin-top: 10px; padding: 8px; background: #222; border-radius: 3px;">
          ...
          <select id="selected-npc-patrol-behavior" style="width: 100%; padding: 3px; font-size: 11px;">
            <option value="loop" ${
              npcData.patrolBehavior === "loop" ? "selected" : ""
            }>Loop (continuous)</option>
            <option value="pingpong" ${
              npcData.patrolBehavior === "pingpong" ? "selected" : ""
            }>Ping-Pong (back and forth)</option>
            <option value="once" ${
              npcData.patrolBehavior === "once" ? "selected" : ""
            }>Once (stop at end)</option>
            <option value="random" ${
              npcData.patrolBehavior === "random" ? "selected" : ""
            }>Random order</option>
          </select>
          <label style="font-size: 11px; margin-top: 5px; display: block;">Wait Time at Points (s):</label>
          <input type="number" id="selected-npc-wait-time" value="${
            npcData.waitTime || 0.5
          }" step="0.1" min="0" 
                 style="width: 100%; padding: 3px; font-size: 11px;"
                 onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
                 oninput="window.editor._onPropertyInputChange()"
          ...
          <div style="margin-top: 5px; font-size: 10px; color: #aaa;">
            Patrol Points: ${
              npcData.patrolPoints ? npcData.patrolPoints.length : 0
            }
          </div>
        </div>
      `;
          ...
          <select id="selected-collider-type" style="width: 100%; padding: 3px; font-size: 11px;">
            ${this.colliderTypes
              .map(
                (type) =>
                  `<option value="${type}" ${
                    type === colliderData.type ? "selected" : ""
                  }>${type.charAt(0).toUpperCase() + type.slice(1)}</option>`
              )
              .join("")}
          </select>
          ...
          <select id="selected-material-type" style="width: 100%; padding: 3px; font-size: 11px;">
            ${this.materialTypes
              .map(
                (type) =>
                  `<option value="${type}" ${
                    type === colliderData.materialType ? "selected" : ""
                  }>${type.charAt(0).toUpperCase() + type.slice(1)}</option>`
              )
              .join("")}
          </select>
      ...
      typeSpecificInputs = `
          ...
          <select id="selected-platform-type" style="width: 100%; padding: 3px; font-size: 11px;">
            ${this.platformTypes
              .map(
                (type) =>
                  `<option value="${type}" ${
                    type === platformData.type ? "selected" : ""
                  }>${type.charAt(0).toUpperCase() + type.slice(1)}</option>`
              )
              .join("")}
          </select>
        ...
        <div style="margin-top: 5px;">
          ...
          <div style="display: flex; gap: 5px;">
            <input type="number" id="selected-platform-width" value="${
              platformData.size[0]
            }" step="0.5" min="0.1" placeholder="W"
                   style="width: 33%; padding: 3px; font-size: 11px;" 
                   onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
                   oninput="window.editor._onPropertyInputChange()"
                   onchange="window.editor._onPropertyInputChange()">
            <input type="number" id="selected-platform-height" value="${
              platformData.size[1]
            }" step="0.1" min="0.1" placeholder="H"
                   style="width: 33%; padding: 3px; font-size: 11px;" 
                   onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
                   oninput="window.editor._onPropertyInputChange()"
                   onchange="window.editor._onPropertyInputChange()">
            <input type="number" id="selected-platform-depth" value="${
              platformData.size[2]
            }" step="0.5" min="0.1" placeholder="D"
                   style="width: 33%; padding: 3px; font-size: 11px;" 
                   onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
                   oninput="window.editor._onPropertyInputChange()"
                   onchange="window.editor._onPropertyInputChange()">
          ...
          <select id="selected-platform-texture" style="width: 100%; padding: 3px; font-size: 11px;">
            ${this.textureTypes
              .map(
                (type) =>
                  `<option value="${type}" ${
                    type === platformData.texture ? "selected" : ""
                  }>${type.charAt(0).toUpperCase() + type.slice(1)}</option>`
              )
              .join("")}
          </select>
        ...
        <div style="margin-top: 5px;">
          ...
          <input type="color" id="selected-platform-color" value="#${platformData.color
            .toString(16)
            .padStart(6, "0")}" 
                 style="width: 100%; padding: 3px; font-size: 11px;">
        </div>
          ...
          <div style="margin-top: 10px; padding: 10px; border: 1px solid #2196F3; border-radius: 3px;">
            ...
            ${
              platformData.type === "moving"
                ? `
              <label style="font-size: 11px;">Speed:</label><br>
              <input type="number" id="anim-speed" value="${
                platformData.animation?.speed || 2
              }" step="0.1" min="0.1"
              ...
              <div id="waypoint-list" style="font-size: 10px; max-height: 100px; overflow-y: auto;">
                ${(platformData.animation?.path || [])
                  .map(
                    (wp, i) => `
                  <div style="margin: 2px 0; padding: 2px; background: #222;">[${wp
                    .map((v) => v.toFixed(1))
                    .join(", ")}] 
            ...
            ${
              ...
              <select id="rotation-axis" style="width: 100%; padding: 3px; font-size: 11px; margin-bottom: 5px;">
                <option value="0,1,0" ${
                  platformData.animation?.rotationAxis?.join(",") === "0,1,0"
                    ? "selected"
                    : ""
                }>Y (Horizontal Spin)</option>
                <option value="1,0,0" ${
                  platformData.animation?.rotationAxis?.join(",") === "1,0,0"
                    ? "selected"
                    : ""
                }>X (Flip Forward/Back)</option>
                <option value="0,0,1" ${
                  platformData.animation?.rotationAxis?.join(",") === "0,0,1"
                    ? "selected"
                    : ""
                }>Z (Roll Side to Side)</option>
              </select>
              <label style="font-size: 11px;">Speed (rad/s):</label><br>
              <input type="number" id="rotation-speed" value="${
                platformData.animation?.rotationSpeed || 1
              }" step="0.1" min="0.1"
                     style="width: 100%; padding: 3px; font-size: 11px;">
            `
                : ""
            ...
            ${
              platformData.type === "disappearing"
                ? `
              <label style="font-size: 11px;">Visible Duration (s):</label><br>
              <input type="number" id="disappear-interval" value="${
                platformData.animation?.disappearInterval || 3
              }" step="0.5" min="0.1"
                     style="width: 100%; padding: 3px; font-size: 11px; margin-bottom: 5px;">
              <label style="font-size: 11px;">Invisible Duration (s):</label><br>
              <input type="number" id="disappear-duration" value="${
                platformData.animation?.disappearDuration || 2
              }" step="0.5" min="0.1"
                     style="width: 100%; padding: 3px; font-size: 11px;">
            `
                : ""
    ...
    return `
      <div id="selection-properties" style="margin-top: 20px; border: 1px solid #444; padding: 10px; border-radius: 3px;">
        <h4 style="margin: 0 0 10px 0; color: #4CAF50;">Selected: ${
        ...
        <div style="margin-bottom: 15px;">
          ...
          <div style="display: flex; gap: 5px;">
            <div>
              ...
              <input type="number" id="selected-pos-x" value="${pos.x.toFixed(
                2
              )}" step="0.1" 
            ...
            <div>
              ...
              <input type="number" id="selected-pos-y" value="${pos.y.toFixed(
                2
              )}" step="0.1" 
                     style="width: 60px; padding: 3px; font-size: 11px;" 
                     onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
            ...
            <div>
              ...
              <input type="number" id="selected-pos-z" value="${pos.z.toFixed(
                2
              )}" step="0.1" 
                     style="width: 60px; padding: 3px; font-size: 11px;" 
                     onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
        ...
        <div style="margin-bottom: 15px;">
          ...
          <div style="display: flex; gap: 5px;">
            <div>
              <label style="font-size: 10px;">X:</label><br>
              <input type="number" id="selected-rot-x" value="${(
                (rot.x * 180) /
                Math.PI
              ).toFixed(1)}" step="1" 
                     style="width: 60px; padding: 3px; font-size: 11px;" 
            ...
            <div>
              <label style="font-size: 10px;">Y:</label><br>
              <input type="number" id="selected-rot-y" value="${(
                (rot.y * 180) /
                Math.PI
              ).toFixed(1)}" step="1" 
                     style="width: 60px; padding: 3px; font-size: 11px;" 
            ...
            <div>
              <label style="font-size: 10px;">Z:</label><br>
              <input type="number" id="selected-rot-z" value="${(
                (rot.z * 180) /
                Math.PI
              ).toFixed(1)}" step="1" 
                     style="width: 60px; padding: 3px; font-size: 11px;" 
  ...
  _getColliderSizeInputs(colliderData) {
    ...
    const colliderTypeSelect = document.getElementById(
      "selected-collider-type"
    );
    const currentType = colliderTypeSelect
      ? colliderTypeSelect.value
      : colliderData.type;
    ...
    if (currentType === "sphere") {
      return `
        <div>
          ...
          <input type="number" id="selected-size-0" value="${
      ...
      return `
        <div style="display: flex; gap: 5px;">
          ...
          <div style="flex: 1;">
            ...
            <input type="number" id="selected-size-1" value="${
              colliderData.size[1] || 2
            }" step="0.1" min="0.1" 
                   style="width: 100%; padding: 3px; font-size: 11px;" 
                   onkeydown="if(event.key==='Enter'){window.editor._applySelectedProperties();return;}event.stopPropagation();" 
                   oninput="window.editor._onPropertyInputChange()"
      ...
      return `
        <div style="display: flex; gap: 5px;">
          ...
          <div style="flex: 1;">
            ...
            <input type="number" id="selected-size-2" value="${
  ...
  _bindUIEvents() {
    ...
    // Collectible type change - show/hide chest contents
const collectibleTypeSelect = document.getElementById('collectible-type');
if (collectibleTypeSelect) {
  collectibleTypeSelect.addEventListener('change', (e) => {
    const chestContents = document.getElementById('chest-contents');
    if (chestContents) {
      chestContents.style.display = e.target.value === 'chest' ? 'block' : 'none';
    }
  });
  // Trigger initial state
  collectibleTypeSelect.dispatchEvent(new Event('change'));
}
    ...
    if (interactiveTypeSelect) {
      interactiveTypeSelect.addEventListener("change", (e) => {
        ...
        if (ppSettings && gpSettings) {
          ppSettings.style.display =
            e.target.value === "pressurePlate" ? "block" : "none";
          gpSettings.style.display =
            e.target.value === "gltfPlatform" ? "block" : "none";
        }
      });
    ...
    const createColliderFromMeshBtn = document.getElementById(
      "create-collider-from-mesh"
    );
    ...
    if (addWaypointBtn) {
      addWaypointBtn.addEventListener("click", (e) => {
        ...
        console.log(
          "Waypoint mode activated. Click on the level to add waypoint."
        );
        this._updateStatus(
          "👆 Click on the level to add a waypoint to the selected platform"
        );
      });
    }

    // Mesh animation type change - show/hide animation settings
    const meshAnimationTypeSelect = document.getElementById(
      "mesh-animation-type"
    );
    if (meshAnimationTypeSelect) {
      meshAnimationTypeSelect.addEventListener("change", (e) => {
        ...
        const rotatingSettings = document.getElementById(
          "mesh-rotating-settings"
        );
        const disappearingSettings = document.getElementById(
          "mesh-disappearing-settings"
        );

        if (movingSettings && rotatingSettings && disappearingSettings) {
          movingSettings.style.display =
            e.target.value === "moving" ? "block" : "none";
          rotatingSettings.style.display =
            e.target.value === "rotating" ? "block" : "none";
          disappearingSettings.style.display =
            e.target.value === "disappearing" ? "block" : "none";
        }
      });
      // Trigger initial change
    ...
    const meshInitializePathBtn = document.getElementById(
      "mesh-initialize-path"
    );
    ...
    if (meshAddWaypointBtn) {
      meshAddWaypointBtn.addEventListener("click", (e) => {
        ...
        console.log(
          "Mesh waypoint mode activated. Click on the level to add waypoint."
        );
        this._updateStatus(
          "👆 Click on the level to add a waypoint to the mesh animation"
        );
      });
    }

    // Apply mesh animation button
    const applyMeshAnimationBtn = document.getElementById(
  ...
  _bindEvents() {
    ...
    this.renderer.domElement.addEventListener(
      "mousedown",
      this.boundEventHandlers.mousedown
    );
    this.renderer.domElement.addEventListener(
      "mousemove",
      this.boundEventHandlers.mousemove
    );
    this.renderer.domElement.addEventListener(
      "mouseup",
    ...
    this.renderer.domElement.addEventListener(
      "wheel",
      this.boundEventHandlers.wheel
    );

    // Prevent right-click context menu
    this.renderer.domElement.addEventListener(
      "contextmenu",
      this.boundEventHandlers.contextmenu
    );
  ...
  _onKeyDown(event) {
    ...
    if (this.keys["NumpadAdd"] || this.keys["Equal"]) {
      // Zoom in (+ or =)
      this.camera.position.multiplyScalar(0.95);
    }
    if (this.keys["NumpadSubtract"] || this.keys["Minus"]) {
      // Zoom out (-)
  ...
  _handlePlacement(event) {
    ...
    const intersects = this.raycaster.intersectObjects(
      [...this.levelGeometry.children, this.groundPlane],
      true
    );
  ...
  _handleSelection(event) {
    ...
    if (this.addingWaypoint && this.mode === "select") {
      ...
      if (intersects.length > 0) {
        ...
        this._updateStatus(
          "⚠️ Click on any visible geometry (level, platform, etc.) to add waypoint"
        );
      }
      return;
    }
  ...
  _handleMeshSelection(event) {
    ...
    if (intersects.length > 0) {
      ...
      const meshInfo = this.levelMeshes.find(
        (info) => info.mesh === clickedMesh
      );
      if (meshInfo) {
  ...
  _addEnemy(position) {
    ...
    const type =
      enemyTypeSelect && enemyTypeSelect.value
    ...
    this._updateUI();
    this._updateStatus(
      `Added ${type} enemy at [${position.x.toFixed(1)}, ${position.y.toFixed(
        1
  ...
  _addNpc(position) {
    ...
    const type =
      npcTypeSelect && npcTypeSelect.value ? npcTypeSelect.value : "yellow_bot";
    ...
    this._updateUI();
    this._updateStatus(
      `Added ${type} NPC at [${position.x.toFixed(1)}, ${position.y.toFixed(
        1
      )}, ${position.z.toFixed(1)}]`
    );
  }
  ...
  _addLight(position) {
    ...
    const type =
      lightTypeSelect && lightTypeSelect.value
    ...
    this._updateUI();
    this._updateStatus(
      `Added ${type} light at [${position.x.toFixed(1)}, ${position.y.toFixed(
        1
      )}, ${position.z.toFixed(1)}]`
    );
  }
  ...
  /**
 * Get HTML for collectibles list
 */
_getCollectiblesListHTML() {
  let html = '';
  ...
  this.collectibles.chests.forEach((chest, index) => {
    html += `
      <div class="item-row" data-type="chest" data-index="${index}">
        ...
        at [${chest.position.join(', ')}]
        <button onclick="window.editor._deleteCollectible('chest', ${index})">Delete</button>
      </div>
    `;
  });
  ...
  this.collectibles.apples.forEach((apple, index) => {
    html += `
      <div class="item-row" data-type="apple" data-index="${index}">
        ...
        at [${apple.position.join(', ')}]
        <button onclick="window.editor._deleteCollectible('apple', ${index})">Delete</button>
      </div>
    `;
  });
  ...
  this.collectibles.potions.forEach((potion, index) => {
    html += `
      <div class="item-row" data-type="potion" data-index="${index}">
        ...
        at [${potion.position.join(', ')}]
        <button onclick="window.editor._deleteCollectible('potion', ${index})">Delete</button>
      </div>
    `;
  });
  ...
  return html || '<div style="font-size: 11px; color: #888; text-align: center;">No collectibles placed yet</div>';
}

/**
 * Create visual representations for collectibles
 */
_createCollectibleVisuals() {
  // Clear existing collectible visuals
  this.collectibleMeshes.forEach(mesh => {
    this.scene.remove(mesh);
    if (mesh.geometry) mesh.geometry.dispose();
    if (mesh.material) mesh.material.dispose();
...
_createChestVisuals() {
  this.collectibles.chests.forEach((chest, index) => {
    ...
    mesh.position.set(chest.position[0], chest.position[1] + 0.6, chest.position[2]);
    mesh.userData = { type: 'chest', index: index, collectibleData: chest };
    mesh.name = `chest_${index}`;
    ...
    lockMesh.position.set(0, 0.3, 0.65);
    mesh.add(lockMesh);
...
_createAppleVisuals() {
  this.collectibles.apples.forEach((apple, index) => {
    ...
    mesh.position.set(apple.position[0], apple.position[1] + 0.4, apple.position[2]);
    mesh.userData = { type: 'apple', index: index, collectibleData: apple };
    mesh.name = `apple_${index}`;
...
/**
 * Create potion visuals
 */
_createPotionVisuals() {
  this.collectibles.potions.forEach((potion, index) => {
    const geometry = new THREE.CylinderGeometry(0.3, 0.25, 0.8, 12);
    const material = new THREE.MeshLambertMaterial({ 
      color: 0x4444ff,
      transparent: true,
      opacity: 0.8,
    ...
    mesh.position.set(potion.position[0], potion.position[1] + 0.4, potion.position[2]);
    mesh.userData = { type: 'potion', index: index, collectibleData: potion };
    mesh.name = `potion_${index}`;
...
_createCoinVisuals() {
  this.collectibles.coins.forEach((coin, index) => {
    ...
    mesh.position.set(coin.position[0], coin.position[1] + 0.05, coin.position[2]);
    mesh.rotation.x = Math.PI / 2; // Lay flat
    mesh.userData = { type: 'coin', index: index, collectibleData: coin };
    mesh.name = `coin_${index}`;
    
    // Add detail
    const detailGeometry = new THREE.CylinderGeometry(0.3, 0.3, 0.11, 8);
    const detailMaterial = new THREE.MeshLambertMaterial({ color: 0xffff00 });
...
_createGemVisuals() {
  this.collectibles.gems.forEach((gem, index) => {
    ...
    mesh.position.set(gem.position[0], gem.position[1] + 0.6, gem.position[2]);
    mesh.userData = { type: 'gem', index: index, collectibleData: gem };
    mesh.name = `gem_${index}`;
    
    // Add inner glow
    const innerGeometry = new THREE.OctahedronGeometry(0.3, 0);
...
_createKeyVisuals() {
  this.collectibles.keys.forEach((key, index) => {
    ...
    // Key handle (circle)
    const handleGeometry = new THREE.TorusGeometry(0.2, 0.05, 8, 12);
    const handleMaterial = new THREE.MeshLambertMaterial({ color: 0xcccccc });
    const handleMesh = new THREE.Mesh(handleGeometry, handleMaterial);
    handleMesh.rotation.x = Math.PI / 2;
    group.add(handleMesh);
    
    // Key shaft
    const shaftGeometry = new THREE.BoxGeometry(0.5, 0.05, 0.05);
    const shaftMaterial = new THREE.MeshLambertMaterial({ color: 0xaaaaaa });
    const shaftMesh = new THREE.Mesh(shaftGeometry, shaftMaterial);
    shaftMesh.position.x = 0.25;
    group.add(shaftMesh);
    
    // Key teeth
    const teethGeometry = new THREE.BoxGeometry(0.1, 0.05, 0.1);
    const teethMaterial = new THREE.MeshLambertMaterial({ color: 0x888888 });
    const teethMesh1 = new THREE.Mesh(teethGeometry, teethMaterial);
    teethMesh1.position.set(0.45, 0.08, 0);
    group.add(teethMesh1);
    
    const teethMesh2 = new THREE.Mesh(teethGeometry, teethMaterial);
    teethMesh2.position.set(0.4, -0.06, 0);
    group.add(teethMesh2);
    
    group.position.set(key.position[0], key.position[1] + 0.2, key.position[2]);
    group.userData = { type: 'key', index: index, collectibleData: key };
    group.name = `key_${index}`;
...
/**
 * Add a collectible at the specified position
 */
_addCollectible(position) {
  const typeSelect = document.getElementById('collectible-type');
  const type = typeSelect ? typeSelect.value : 'apple';
  ...
  if (type === 'chest') {
    const contentsSelect = document.getElementById('chest-contents-type');
    const contents = contentsSelect ? contentsSelect.value : 'apple';
    collectibleData.contents = contents;
    this.collectibles.chests.push(collectibleData);
  } else {
    this.collectibles[type + 's'].push(collectibleData);
  }
  
  this._createCollectibleVisuals();
  this._updateUI();
  this._updateStatus(`Added ${type} at [${position.x.toFixed(1)}, ${position.y.toFixed(1)}, ${position.z.toFixed(1)}]`);
}

/**
 * Delete a collectible
 */
_deleteCollectible(type, index) {
  if (this.collectibles[type] && index >= 0 && index < this.collectibles[type].length) {
    this.collectibles[type].splice(index, 1);
    this._createCollectibleVisuals();
    this._updateUI();
    this._updateStatus(`Deleted ${type}`);
  }
}

/**
 * Load collectibles from level data
 */
_loadCollectibles() {
  ...
  _addPatrolPoint(position) {
    if (
      !this.selected ||
      (this.selectedType !== "enemy" && this.selectedType !== "npc")
    ) {
    ...
    if (this.selectedType === "enemy") {
      ...
      this._updateUI();
      this._updateStatus(
        `Added patrol point ${pointIndex} for enemy ${enemyIndex}`
      );
    } else if (this.selectedType === "npc") {
      ...
      this._updateUI();
      this._updateStatus(
        `Added patrol point ${pointIndex} for NPC ${npcIndex}`
      );
    }
  }
  ...
  _addCollider(position) {
    ...
    this._updateUI();
    this._updateStatus(
      `Added ${type} collider at [${position.x.toFixed(
        1
  ...
  _createColliderFromSelectedMesh() {
    ...
    if (colliderType === "mesh") {
      ...
      this._updateStatus(
        `Created Box collider for: ${meshName} (${size.x.toFixed(
          2
  ...
  _deleteCollider(index) {
    if (index >= 0 && index < this.colliders.length) {
      ...
      if (
        this.selected &&
        this.selectedType === "collider" &&
        this.selected.userData.index === index
      ) {
        this.selected = null;
        this.selectedType = null;
      }
  ...
  _editCollider(index) {
    if (index >= 0 && index < this.colliders.length) {
      ...
      this._updateStatus(
        `Editing collider: ${collider.id}. Use properties panel to modify.`
      );
    }
  }
  ...
  _addWaypointToSelectedPlatform(position) {
    ...
    this._updateUI();
    this._updateStatus(
      `✅ Added waypoint ${platformData.animation.path.length} to platform`
    );
    this.addingWaypoint = false;
  }
  ...
  _createInteractiveObjectVisuals() {
    ...
    this.interactiveObjects.forEach((objData, index) => {
      ...
      if (objData.objectType === "pressurePlate") {
        ...
        mesh.position.set(...objData.position);
        if (objData.scale) {
          const scale = Array.isArray(objData.scale)
            ? objData.scale[0]
            : objData.scale;
  ...
  _addInteractiveObject(position) {
    ...
    if (objectType === "pressurePlate") {
      ...
      objData.activationWeight = weightInput
        ? parseFloat(weightInput.value)
        : 50;
      objData.pressedHeight = pressedHeightInput
        ? parseFloat(pressedHeightInput.value)
        : -0.1;
      objData.color = colorInput
        ? parseInt(colorInput.value.replace("#", "0x"))
        : 0x00ff00;
  ...
  _deleteInteractiveObject(index) {
    if (index >= 0 && index < this.interactiveObjects.length) {
      ...
      this.triggers = this.triggers.filter(
        (t) => t.sourceId !== obj.id && t.targetId !== obj.id
      );
  ...
  _addPlaceableBlock(position) {
    ...
    this._updateUI();
    this._updateStatus(
      `✅ Added ${colorName} block at [${position.x.toFixed(
        1
      )}, ${position.y.toFixed(1)}, ${position.z.toFixed(1)}]`
    );
  }
  ...
  _createTriggerConnectionVisuals() {
    ...
    this.triggers.forEach((trigger) => {
      const sourceObj = this.interactiveObjects.find(
        (o) => o.id === trigger.sourceId
      );
      const targetObj = this.interactiveObjects.find(
        (o) => o.id === trigger.targetId
      );
  ...
  _deleteTrigger(index) {
    if (index >= 0 && index < this.triggers.length) {
      ...
      this._updateUI();
      this._updateStatus(
        `Deleted trigger: ${trigger.sourceId} → ${trigger.targetId}`
      );
    }
  }
  ...
  _selectObject(object) {
    ...
    if (
      ...
      this.selected.material.emissive
    ) {
  ...
  _applySelectedProperties() {
    ...
    if (posX && posY && posZ) {
      ...
      if (this.selectedType === "enemy" && this.enemies[index]) {
        ...
        if (
          patrolData.entityType === "enemy" &&
          this.enemies[patrolData.enemyIndex]
        ) {
          const enemy = this.enemies[patrolData.enemyIndex];
          if (enemy.patrolPoints && enemy.patrolPoints[patrolData.pointIndex]) {
            enemy.patrolPoints[patrolData.pointIndex][0] = parseFloat(
              posX.value
            );
            enemy.patrolPoints[patrolData.pointIndex][1] = parseFloat(
              posY.value
            );
            enemy.patrolPoints[patrolData.pointIndex][2] = parseFloat(
              posZ.value
            );
          }
        } else if (
          patrolData.entityType === "npc" &&
          this.npcs[patrolData.npcIndex]
        ) {
    ...
    if (this.selectedType === "enemy") {
      ...
      if (this.enemies[index]) {
        if (enemyTypeSelect) {
          ...
          this.enemies[index].modelUrl = this._getDefaultModelUrl(
            enemyTypeSelect.value
          );

          // Update visual color
          if (this.selected.material && this.selected.material.color) {
            this.selected.material.color.setHex(
              this._getEnemyColor(enemyTypeSelect.value)
            );
          }
        }
        if (speedInput) {
      ...
      const patrolBehaviorSelect = document.getElementById(
        "selected-npc-patrol-behavior"
      );
      ...
      if (this.npcs[index]) {
        if (npcTypeSelect) {
          ...
          this.npcs[index].modelUrl = this._getDefaultModelUrl(
            npcTypeSelect.value
          );
        }
        if (speedInput) {
      ...
      const materialTypeSelect = document.getElementById(
      ...
      if (collider) {
        ...
        if (
          materialTypeSelect &&
          materialTypeSelect.value !== collider.materialType
        ) {
      ...
      const platformTypeSelect = document.getElementById(
      ...
      if (platform) {
        ...
        if (platformTypeSelect && platformTypeSelect.value !== platform.type) {
          ...
          if (platform.type !== "static" && !platform.animation) {
            platform.animation = {
              ...
              path: platform.type === "moving" ? [] : undefined,
              rotationAxis:
                platform.type === "rotating" ? [0, 1, 0] : undefined,
              rotationSpeed: platform.type === "rotating" ? 1.0 : undefined,
              disappearInterval:
                platform.type === "disappearing" ? 3.0 : undefined,
              disappearDuration:
                platform.type === "disappearing" ? 2.0 : undefined,
            };
        ...
        if (platform.animation) {
          if (platform.type === "moving") {
            ...
            if (speedInput)
              platform.animation.speed = parseFloat(speedInput.value);
            ...
            if (axisSelect) {
              platform.animation.rotationAxis = axisSelect.value
                .split(",")
                .map(parseFloat);
            }
            if (rotSpeedInput) {
              platform.animation.rotationSpeed = parseFloat(
                rotSpeedInput.value
              );
            }
            ...
            if (intervalInput)
              platform.animation.disappearInterval = parseFloat(
                intervalInput.value
              );
            if (durationInput)
              platform.animation.disappearDuration = parseFloat(
                durationInput.value
              );
          }
        }
    ...
    if (this.selectedType === "collider") {
      ...
      if (collider) {
        ...
        if (sizeInputs.length > 0) {
          ...
          this._updateColliderVisualSize(
            this.selected,
            collider.type,
            tempSize
          );
  ...
  _saveLevel() {
    ...
    this.currentLevel.collectibles = { ...this.collectibles };
    ...
    this._updateStatus(
      "Level data saved! Replace src/game/levelData.js with the downloaded file."
    );
  }
...
export const levels = ${levelsJSON};`;
  ...
  _applyMeshAnimation() {
    ...
    const animationType =
      document.getElementById("mesh-animation-type")?.value || "none";
    ...
    let animData = this.meshAnimations.find(
      (a) => a.meshName === this.selectedMesh.name
    );
    if (!animData) {
      animData = {
    ...
    switch (animationType) {
      case "moving":
        const speed = parseFloat(
          document.getElementById("mesh-anim-speed")?.value || "2"
        );
        const loopBehavior =
          document.getElementById("mesh-loop-behavior")?.value || "loop";

        // Check if path was initialized
        if (!animData.data.path || animData.data.path.length === 0) {
          alert(
            '⚠️ Please initialize the path first (click "Initialize Path" button)'
          );
          return;
        }
      ...
      case "rotating":
        const axisStr =
          document.getElementById("mesh-rotation-axis")?.value || "0,1,0";
        const [ax, ay, az] = axisStr.split(",").map(Number);
        const rotSpeed = parseFloat(
          document.getElementById("mesh-rotation-speed")?.value || "1"
        );
        animData.data = {
          axis: [ax, ay, az],
          speed: rotSpeed,
      ...
      case "disappearing":
        const interval = parseFloat(
          document.getElementById("mesh-disappear-interval")?.value || "3"
        );
        const duration = parseFloat(
          document.getElementById("mesh-disappear-duration")?.value || "2"
        );
        animData.data = {
    ...
    this._updateUI();
    this._updateStatus(
      `✅ Animation "${animationType}" applied to mesh "${this.selectedMesh.name}"`
    );
  ...
  _addMeshWaypoint(position) {
    ...
    if (!animData.data.path || animData.data.path.length === 0) {
      this._updateStatus(
        '⚠️ Path not initialized - click "Initialize Path" button first'
      );
      this.addingMeshWaypoint = false;
      return;
    }
    ...
    this._updateStatus(
      `✅ Waypoint ${animData.data.path.length} added (${
        animData.data.path.length - 1
      } points after start)`
    );
    this.addingMeshWaypoint = false;
    console.log(
      "Waypoint added to mesh:",
      waypoint,
      "Total points:",
      animData.data.path.length
    );
  }
  ...
  _initializeMeshPath() {
    ...
    this._updateStatus(
      `✅ Path initialized for "${
        this.selectedMesh.name
      }" from position [${startPosition.join(", ")}]`
    );
  ...
  _updateMeshWaypointList(animData) {
    ...
    waypointList.innerHTML = animData.data.path
      .map((wp, index) => {
        ...
        return `
        <div style="margin: 3px 0; padding: 4px; background: ${bgColor}; border-left: 3px solid ${
          isStart ? "#4CAF50" : "#2196F3"
        }; border-radius: 2px; display: flex; justify-content: space-between; align-items: center;">
          <span style="font-size: 10px;"><strong>${label}:</strong> [${wp
          .map((v) => v.toFixed(1))
          .join(", ")}]</span>
          ${
            canDelete
    ...
    loopInfo.style.cssText =
      "margin-top: 5px; padding: 4px; background: #2a2a2a; border-radius: 2px; font-size: 9px; color: #aaa;";
  ...
  _deleteMeshWaypoint(meshName, waypointIndex) {
    ...
    if (waypointIndex === 0) {
      this._updateStatus(
        '⚠️ Cannot delete start position - use "Initialize Path" to reset'
      );
      return;
    }
    ...
    this._updateUI();
    this._updateStatus(
      `✅ Waypoint ${waypointIndex} removed from "${meshName}"`
    );
  }
